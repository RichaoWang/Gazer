...
namespace Render {
    ...
    void DrawFovCircle(const CEntity &LocalEntity) {
        ...
        float Radius = tan(AimControl::AimFov / 180.f * M_PI / 2.f) / tan(LocalEntity.Pawn.Fov / 180.f * M_PI / 2.f) *
                       Gui.Window.Size.x;
        float MinRadius =
                tan(AimControl::AimFovMin / 180.f * M_PI / 2.f) / tan(LocalEntity.Pawn.Fov / 180.f * M_PI / 2.f) *
                Gui.Window.Size.x;
    ...
    void DrawCrossHair(ImDrawList *drawList, const ImVec2 &pos, ImU32 color) noexcept {
        ...
        */
        if (CrosshairsCFG::drawOutLine) {
            ...
            if (CrosshairsCFG::drawDot)
                drawList->AddRectFilled(ImVec2(pos.x - offset1.x, pos.y - offset1.y),
                                        ImVec2(pos.x + offset2.x, pos.y + offset2.y), color & IM_COL32_A_MASK);

            if (CrosshairsCFG::drawCrossline) {
                //left
                drawList->AddRectFilled(
                        ImVec2(pos.x - (outlineGap + BorderWidth + CrosshairsCFG::HorizontalLength), pos.y - Thickness),
                        ImVec2(pos.x - outlineGap, pos.y + 1 + Thickness), color & IM_COL32_A_MASK);
                //right
                drawList->AddRectFilled(ImVec2(pos.x + (outlineGap + DotSize), pos.y - Thickness), ImVec2(pos.x +
                                                                                                          (outlineGap +
                                                                                                           DotSize +
                                                                                                           BorderWidth +
                                                                                                           CrosshairsCFG::HorizontalLength),
                                                                                                          pos.y + 1 +
                                                                                                          Thickness),
                                        color & IM_COL32_A_MASK);
                //top
                if (!CrosshairsCFG::tStyle)
                    drawList->AddRectFilled(ImVec2(pos.x - Thickness,
                                                   pos.y - (outlineGap + BorderWidth + CrosshairsCFG::VerticalLength)),
                                            ImVec2(pos.x + 1 + Thickness, pos.y - outlineGap), color & IM_COL32_A_MASK);
                //bottom
                drawList->AddRectFilled(ImVec2(pos.x - Thickness, pos.y + outlineGap + DotSize),
                                        ImVec2(pos.x + 1 + Thickness, pos.y + (outlineGap + DotSize + BorderWidth +
                                                                               CrosshairsCFG::VerticalLength)),
                                        color & IM_COL32_A_MASK);
            }

            //circle
            if (CrosshairsCFG::drawCircle)
                drawList->AddCircle(ImVec2(pos.x, pos.y), CrosshairsCFG::CircleRadius, color & IM_COL32_A_MASK, 0,
                                    3.0f);
        }
        ...
        */
        // dot
        if (CrosshairsCFG::drawDot)
            drawList->AddRectFilled(ImVec2(pos.x - offset1.x + DotSize, pos.y - offset1.y + DotSize),
                                    ImVec2(pos.x + offset1.x, pos.y + offset1.y), color);

        if (CrosshairsCFG::drawCrossline) {
            // left
            drawList->AddRectFilled(ImVec2(pos.x - (gap + CrosshairsCFG::HorizontalLength), pos.y - Thickness + 1),
                                    ImVec2(pos.x - gap, pos.y + Thickness), color);
            // right
            drawList->AddRectFilled(ImVec2(pos.x + gap + DotSize, pos.y - Thickness + 1),
                                    ImVec2(pos.x + (gap + DotSize + CrosshairsCFG::HorizontalLength),
                                           pos.y + Thickness), color);
            // top
            if (!CrosshairsCFG::tStyle)
                drawList->AddRectFilled(ImVec2(pos.x - Thickness + 1, pos.y - (gap + CrosshairsCFG::VerticalLength)),
                                        ImVec2(pos.x + Thickness, pos.y - gap), color);
            // bottom
            drawList->AddRectFilled(ImVec2(pos.x - Thickness + 1, pos.y + gap + DotSize),
                                    ImVec2(pos.x + Thickness, pos.y + (gap + DotSize + CrosshairsCFG::VerticalLength)),
                                    color);
        }
    ...
    void DrawCrossHair2D() {
        ...
        Gui.Line({SightPos.x - CrosshairsCFG::CrossHairSize, SightPos.y},
                 {SightPos.x + CrosshairsCFG::CrossHairSize, SightPos.y}, CrosshairsCFG::CrossHairColor, 1);
        Gui.Line({SightPos.x, SightPos.y - CrosshairsCFG::CrossHairSize},
                 {SightPos.x, SightPos.y + CrosshairsCFG::CrossHairSize}, CrosshairsCFG::CrossHairColor, 1);
    }
    ...
    void LineToEnemy(ImVec4 Rect, ImColor Color, float Thickness) {
        ...
        switch (ESPConfig::LinePos) {
            case 0:
                ...
                break;
            case 1:
                Gui.Line({Rect.x + Rect.z / 2, Rect.y}, {Gui.Window.Size.x / 2, Gui.Window.Size.y / 2}, Color,
                         Thickness);
                break;
            case 2:
                Gui.Line({Rect.x + Rect.z / 2, Rect.y + Rect.w}, {Gui.Window.Size.x / 2, Gui.Window.Size.y}, Color,
                         Thickness);
                break;
            default:
                break;
    ...
    void HeadShootLine(const CEntity &LocalEntity, ImColor Color) {
        ...
        Pos.y = Gui.Window.Size.y / 2.0f - Gui.Window.Size.y / (2.0f * std::sin(LocalEntity.Pawn.Fov * M_PI / 180.0f) /
                                                                std::sin(90.0f * M_PI / 180.0f)) *
                                           std::sin(LocalEntity.Pawn.ViewAngle.x * M_PI / 180.0f) /
                                           std::sin(90.0f * M_PI / 180.0f);
    ...
    void
    HealthBar::ArmorBarV(bool HasHelmet, float MaxArmor, float CurrentArmor, ImVec2 Pos, ImVec2 Size, bool ShowNum) {
        auto InRange = [&](float value, float min, float max) -> bool {
            return value > min && value <= max;
        };
